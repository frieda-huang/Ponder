...
class CausalSelfAttention(nn.Module):
    def __init__(self, config) -> None:
        ...
        # c_attn: In (B, T, n_embd) Out (B, T, 3 * n_embd)
        # c_proj; In (B, T, n_embd) Out (B, T, n_head)
    ...
    def forward(self, x):
        ...
        # # Flash attention implements these 4 lines really quickly
        # att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
        # att = att.masked_fill_(self.bias[:, :, :T, :T] == 0, float("-inf"))
        # att = F.softmax(att, dim=-1)
        # y = att @ v

        # This is the flash attention part
...
class DataLoaderLite:
    def __init__(self, B, T):
        ...
        with open("/Users/friedahuang/Documents/Ponder/src/llm/input.txt", "r") as f:
            text = f.read()
